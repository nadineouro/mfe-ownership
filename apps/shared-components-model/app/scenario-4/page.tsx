...
import {
  Scenario4DeliveryBox as DeliveryBox,
  Scenario4PickupBox as PickupBox,
} from "@nadineouro/shipping-components";
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setProduct(null);
        setShippingOptions({ options: [], pickups: [] });
      } finally {
        setLoading(false);
      }
    }
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: 16 }}>
          <DeliveryBox options={shippingOptions.options} />
          <PickupBox pickups={shippingOptions.pickups} />
        </div>
      </div>
    </div>
  );
